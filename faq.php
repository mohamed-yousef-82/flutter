...
while ($row = mysqli_fetch_assoc($query)){
  $r= '<tr><td>'.$row['ques'].'</td><td>'.$row['ans'].'</td>';
	$r.=' <td><button class="btn btn-warning" data-toggle="modal" type="button" data-target="#update_modal_'.$row['id'].'"><span class="glyphicon glyphicon-edit"></span> Edit</button></td>';
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
			<form>
				<div class="form-group">
				<label>Question</label>
				 <input class="form-control" name="ques" id="ques" value="<?php echo $row['ques'] ?>" />
				</div>
				<div class="form-group">
				<label>answer</label>
				<input class="form-control" name="ans" id="ans" value="<?php echo $row['ans'] ?>" />
				</div>
			</form>
			</div>
